...
class Filtering(Scene):
    def construct(self):
        ...
        # -----Title-----
        title = MarkupText('Filtering with AND (&amp;)', color=GOLD).scale(0.7).shift(3.5 * UP)
        ...
        subtitle_2 = create_subtitle("Next, we filter the DataFrame to show Engineers under 30 years old.")
        console_2 = create_console(13.9, 0.6, 0.6 * DOWN, DARK_GREY, 'df[(df["Title"] == "Engineer") & (df["Age"] < 30)]')

        self.add(subtitle_2)
        self.play(FadeIn(console_2))
        self.wait(1)

        highlight_rectangles = highlight_rows([dataframe_rows[i] for i in [1, 6]], range(6), YELLOW)
        ...
        filtered_index_column = create_index_column(["0", "5"], filtered_rows[1:])

        self.play(FadeIn(VGroup(*filtered_rows[0])))
        for original_row, new_row in zip([dataframe_rows[i] for i in [1, 6]], filtered_rows[1:]):
