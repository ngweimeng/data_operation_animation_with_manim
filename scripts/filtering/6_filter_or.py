...
class Filtering(Scene):
    def construct(self):
        ...
        # -----Title-----
        title = MarkupText('Filtering with OR', color=GOLD).scale(0.7).shift(3.5 * UP)
        ...
        # -----Resultant Dataframe: Filter using "OR"-----
        subtitle_2 = create_subtitle("Next, we filter the DataFrame to show employees whose title are Engineer or Manager.")
        console_2 = create_console(13.9, 0.6, 0.6 * DOWN, DARK_GREY, 'df[(df["Title"] == "Engineer") OR (df["Title"] == "Manager")]')

        self.add(subtitle_2)
        self.play(FadeIn(console_2))
        self.wait(1)

        highlight_rectangles = highlight_rows([dataframe_rows[i] for i in [1, 2, 4, 6]], range(6), YELLOW)
        ...
        filtered_index_column = create_index_column(["0", "1", "3", "5"], filtered_rows[1:])

        self.play(FadeIn(VGroup(*filtered_rows[0]))) 
        for original_row, new_row in zip([dataframe_rows[i] for i in [1, 2, 4, 6]], filtered_rows[1:]):
