...
class Filtering(Scene):
    def construct(self):
        ...
        # -----Title-----
        title = MarkupText('Filter with Comparison Operator: Greater Than (>)', color=GOLD).scale(0.7).shift(3.5 * UP)
        ...
        subtitle_2 = create_subtitle("Next, we filter the DataFrame to show employees with a Salary greater than 90000.")
        console_2 = create_console(13.9, 0.6, 0.6 * DOWN, DARK_GREY, 'df[df["Salary"] > 90000]')
        ...
        filtered_index_column = create_index_column(["1", "3"], filtered_rows[1:])

        self.play(FadeIn(VGroup(*filtered_rows[0])))
        for original_row, new_row in zip([dataframe_rows[i] for i in [2, 4]], filtered_rows[1:]):
