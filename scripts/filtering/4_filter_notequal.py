...
class Filtering(Scene):
    def construct(self):
        ...
        # -----Title-----
        title = MarkupText('Filtering with Comparison Operator: Not Equal (!=)', color=GOLD).scale(0.7).shift(3.5 * UP)
        ...
        # -----Resultant Dataframe: Filter using "!="-----
        subtitle_2 = create_subtitle("Next, we filter the DataFrame to show employees with are not Engineers.")
        console_2 = create_console(13.9, 0.6, 0.6 * DOWN, DARK_GREY, 'df[df["Title"] != "Engineer"]')

        self.add(subtitle_2)
        self.play(FadeIn(console_2))
        self.wait(1)

        highlight_rectangles = highlight_rows([dataframe_rows[i] for i in [3, 4, 5]], range(6), YELLOW)
        ...
        filtered_index_column = create_index_column(["2", "3", "4"], filtered_rows[1:])

        self.play(FadeIn(VGroup(*filtered_rows[0])))
        for original_row, new_row in zip([dataframe_rows[i] for i in [3, 4, 5]], filtered_rows[1:]):
