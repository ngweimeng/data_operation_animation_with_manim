...
class Select(Scene):
    def construct(self):
        ...
        # -----Title-----
        title = MarkupText('Selecting Rows with ".iloc"', color=GOLD).scale(0.7).shift(3.5 * UP)
        ...
        subtitle_2 = create_subtitle("Next, we select the rows for 'Alice' and 'Diana' using their indices.")
        console_2 = create_console(13.9, 0.6, 0.6 * DOWN, DARK_GREY, 'df.iloc[[0, 3]]')

        self.add(subtitle_2)
        self.play(FadeIn(console_2))
        self.wait(1)

        # Highlight the rows corresponding to 'Alice' (index 0) and 'Diana' (index 3)
        highlight_rectangles = highlight_rows([dataframe_rows[i] for i in [1, 4]], range(6), YELLOW)
        self.play(*[Create(highlight) for highlight in highlight_rectangles])

        # Resultant DataFrame with selected rows
        filtered_data = [
            ["Alice", "28", "Female", "Engineer", "85000", "5"],
            ["Diana", "40", "Female", "Manager", "105000", "10"],
        ]
        ...
        filtered_index_column = create_index_column(["0", "3"], filtered_rows[1:])

        self.play(FadeIn(VGroup(*filtered_rows[0])))
        for original_row, new_row in zip([dataframe_rows[i] for i in [1, 4]], filtered_rows[1:]):
