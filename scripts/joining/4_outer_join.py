...
class Join(Scene):
    def construct(self):
        ...
        # -----Title-----
        title = MarkupText('Outer Join', color=GOLD).scale(0.7).shift(3.5 * UP)
        ...
        self.play(FadeOut(subtitle_1))
         ...
         # -----Joining on "Name"-----
        subtitle_2 = create_subtitle("Next, we join the two DataFrames on the 'Name' column using an Outer Join.")
        console_2 = create_console(13.9, 0.6, 0.3 * DOWN, DARK_GREY, 'pd.merge(df_employee, df_salary, on="Name", how="outer")')
        ...
        resultant_dataframe_rows = create_dataframe(headers3, data3, -0.9 * UP)
        ...
        # Play the transformations for the last 2 rows separately
        row_index = 5 
        animations = [
            TransformFromCopy(dataframe_rows_1[row_index][i], resultant_dataframe_rows[row_index][i])
            for i in range(3)  
        ] + [
        ...
        animations = [
            TransformFromCopy(dataframe_rows_2[row_index][i], resultant_dataframe_rows[row_index + 1][i + 2])
            for i in range(1, 3)  
        ] + [
            FadeIn(resultant_dataframe_rows[row_index + 1][i])
            for i in range(1, 3) 
        ] + [
            TransformFromCopy(dataframe_rows_2[row_index][0], resultant_dataframe_rows[row_index + 1][0])
        ]
        self.play(*animations, run_time=1.75)
