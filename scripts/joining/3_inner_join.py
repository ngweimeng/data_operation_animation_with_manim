...
class Join(Scene):
    def construct(self):
        ...
        # -----Title-----
        title = MarkupText('Inner Join', color=GOLD).scale(0.7).shift(3.5 * UP)
        ...
        self.play(FadeOut(subtitle_1))
         ...
         # -----Joining on "Name"-----
        subtitle_2 = create_subtitle("Next, we join the two DataFrames on the 'Name' column using an Inner Join.")
        console_2 = create_console(13.9, 0.6, 0.6 * DOWN, DARK_GREY, 'pd.merge(df_employee, df_salary, on="Name", how="inner")')
        ...
        resultant_index_column = create_index_column(["0", "1", "2", "3"], resultant_dataframe_rows[1:])
