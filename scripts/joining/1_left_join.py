...
class Join(Scene):
    def construct(self):
        ...
        # -----Title-----
        title = MarkupText('Left Join', color=GOLD).scale(0.7).shift(3.5 * UP)
        ...
        self.play(FadeOut(subtitle_1))
         ...
         # -----Joining on "Name"-----
        subtitle_2 = create_subtitle("Next, we join the two DataFrames on the 'Name' column using a Left Join.")
        console_2 = create_console(13.9, 0.6, 0.6 * DOWN, DARK_GREY, 'pd.merge(df_employee, df_salary, on="Name", how="left")')
        ...
        animations = [
            ...
            for i in range(3)  # Loop over the first three columns (Name, Age, Gender)
        ] + [
            FadeIn(resultant_dataframe_rows[row_index][i])
            for i in range(3, 5)  # Loop over the 4th and 5th columns (Title, Salary)
        ]
        ...
        subtitle_3 = create_subtitle("A left join includes all rows from the left table, but only the matching rows from the right table.")
